...
class AdvancedGraphRAGSystem:
    ...
    def initialize_system(self):
        ...
        try:
            ...
            self.reranker = RerankerModule(
                ...
                embedding_model=self.index_module.embeddings  # 复用已加载的embedding做余弦粗筛
            )
            ...
            # 11. RAGAS 数据收集器
            print("初始化RAGAS数据收集器...")
            from rag_modules.ragas_eval import RagasDataCollector
            self.ragas_collector = RagasDataCollector(config=self.config)
            ...
            if self.index_module.has_collection():
                ...
                if self.index_module.load_collection():
                    ...
                    if self.config.use_parent_child:
                        print("使用 Parent-Child 切分策略...")
                        chunks = self.data_module.chunk_documents_parent_child(
    ...
    def ask_question_with_routing(self, question: str, stream: bool = False, explain_routing: bool = False):
        ...
        try:
            # 1. 智能路由检索
            print("执行智能查询路由...")
            relevant_docs, analysis, pre_rerank_docs = self.query_router.route_query(question, self.config.top_k)
