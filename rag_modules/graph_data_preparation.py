...
class GraphDataPreparationModule:
    ...
    def chunk_documents_parent_child(
        ...
        """
        Parent-Child 切分策略：
        - Parent 块：按章节/语义块切分（大 chunk，保留完整上下文）
        - Child 块：对每个 Parent 按段落/长度切分（小 chunk，精确匹配用）
        - 返回 Parent + Child 混合列表：
          * Parent 块（doc_type="parent"）：用于 Parent 缓存扩展上下文
          * Child 块（doc_type="child"）：用于 Milvus 向量存储和 BM25 检索
        - build_vector_index 时只存 Child 块（过滤 doc_type != "parent"）

        Args:
            parent_chunk_size: Parent 块最大字符数
            child_chunk_size: Child 块最大字符数
            child_chunk_overlap: Child 块重叠字符数

        Returns:
            Parent + Child 混合列表（metadata 中 parent_id 指向所属 Parent 的 chunk_id）
        """
        logger.info(
            f"正在进行 Parent-Child 文档分块，"
            f"Parent 大小: {parent_chunk_size}, Child 大小: {child_chunk_size}, "
            f"Child 重叠: {child_chunk_overlap}"
        )
        ...
        parent_chunk_id_counter = 0
        child_chunk_id_counter = 0
        ...
        for doc in self.documents:
            ...
            parent_chunks = self._split_into_parent_chunks(
                content, parent_chunk_size
            )

            # ========== 第 2 步：对每个 Parent 切分 Child 块 ==========
            for parent_idx, parent_content in enumerate(parent_chunks):
                parent_cid = f"{doc.metadata['node_id']}_parent_{parent_chunk_id_counter}"
                parent_chunk_id_counter += 1

                # 创建 Parent 块（用于缓存扩展上下文）
                parent_doc = Document(
                ...
                child_chunks_from_parent = self._split_into_child_chunks(
                    parent_content, child_chunk_size, child_chunk_overlap
                )

                # 更新 Parent 的 total_child_chunks
                parent_doc.metadata["total_child_chunks"] = len(child_chunks_from_parent)

                for child_idx, child_content in enumerate(child_chunks_from_parent):
                    child = Document(
                        page_content=child_content,
                        metadata={
                            **doc.metadata,
                            "chunk_id": f"{doc.metadata['node_id']}_child_{child_chunk_id_counter}",
                            "parent_id": parent_cid,
                            "parent_chunk_size": len(parent_content),
                            "chunk_index": child_idx,
                            "total_child_chunks": len(child_chunks_from_parent),
                    ...
                    child_chunk_id_counter += 1

        self.chunks = all_chunks
        child_count = sum(1 for c in all_chunks if c.metadata.get("doc_type") == "child")
        parent_count = sum(1 for c in all_chunks if c.metadata.get("doc_type") == "parent")
        logger.info(
            f"Parent-Child 文档分块完成，"
            f"共 {parent_count} 个 Parent 块 + {child_count} 个 Child 块"
        )
        return all_chunks

    def _split_into_parent_chunks(self, content: str, max_size: int) -> List[str]:
        """
        按语义边界（章节标题）切分 Parent 块。
        优先按 ## 标题切分，如果某段超过 max_size 则按段落递归切分。
        """
        sections = content.split('\n## ')
        if len(sections) <= 1:
            # 没有二级标题，按段落切分
        ...
        for i, section in enumerate(sections):
            ...
            # 如果当前块加上新段落后不超过上限，就合并
            if len(current_chunk) + len(section_text) + 2 <= max_size:
                if current_chunk:
                    current_chunk += "\n\n" + section_text
                else:
                ...
                # 如果新段落本身就超过上限，按段落递归切分
                if len(section_text) > max_size:
                    parent_chunks.extend(
                        self._split_by_paragraphs(section_text, max_size)
                    )
                    current_chunk = ""
                else:
                    current_chunk = section_text
    ...
    def _split_into_child_chunks(
        self, parent_content: str, max_size: int, overlap: int
    ) -> List[str]:
        """
        对 Parent 块切分 Child 块。
        优先按段落边界切分，不足时按长度强制切分。
        """
        if len(parent_content) <= max_size:
        ...
        if len(paragraphs) <= 1:
            # 没有段落分隔，按长度强制切分
            return self._force_split(parent_content, max_size, overlap)
        ...
        for para in paragraphs:
            if len(current) + len(para) + 2 <= max_size:
                ...
                if current:
                    child_chunks.append(current)
                if len(para) > max_size:
                    child_chunks.extend(self._force_split(para, max_size, overlap))
                    current = ""
                else:
                    current = para
    ...
    def _force_split(
        self, text: str, max_size: int, overlap: int = 0
    ) -> List[str]:
        """按长度强制切分文本（用于没有语义边界的情况）。"""
        if len(text) <= max_size:
            return [text]

        chunks = []
        step = max_size - overlap if overlap > 0 else max_size
        start = 0
