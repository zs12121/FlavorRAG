...
class MilvusIndexConstructionModule:
    ...
    def _create_collection_schema(self) -> CollectionSchema:
        ...
        fields = [
            ...
            FieldSchema(name="parent_id", dtype=DataType.VARCHAR, max_length=150)
        ]
    ...
    def build_vector_index(self, chunks: List[Document]) -> bool:
        ...
        Args:
            chunks: 文档块列表（会自动过滤掉 Parent 块，只存 Child 块）
            
        Returns:
            是否构建成功
        """
        # 过滤掉 Parent 块（Parent-Child 策略中 Parent 只用于缓存，不存向量库）
        vector_chunks = [c for c in chunks if c.metadata.get("doc_type") != "parent"]
        if not vector_chunks:
            # 兼容旧数据：如果没有 doc_type 字段，全部存入
            vector_chunks = chunks

        logger.info(f"正在构建Milvus向量索引，总块数: {len(chunks)}, 向量存储块数: {len(vector_chunks)}...")
        ...
        try:
            ...
            texts = [chunk.page_content for chunk in vector_chunks]
            vectors = self.embeddings.embed_documents(texts)
            
            # 3. 准备插入数据
            entities = []
            for i, (chunk, vector) in enumerate(zip(vector_chunks, vectors)):
                entity = {
            ...
            logger.info(f"向量索引构建完成，包含 {len(vector_chunks)} 个向量")
            return True
